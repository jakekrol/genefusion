...
parser.add_argument('--prefix', type=str, default='shard_', help='Prefix for shard directories')
...
for i in range(args.n_shards):
    os.makedirs(os.path.join(args.output, f'{args.prefix}{i}'), exist_ok=True)
...
shards = [os.path.join(args.output, f'{args.prefix}{i}') for i in range(args.n_shards)]
meta = {f'{args.prefix}{i}': [] for i in range(args.n_shards)}
while len(inputs) > 0: # until all inputs are assigned
    for i in range(args.n_shards): # loop over shards
        ...
        shard_path = os.path.join(shards[i], os.path.basename(f))
        shutil.copy(f, shard_path) # assign it to the shard
        print(f'Assigned {f} to {shard_path}')
        meta[f'{args.prefix}{i}'] += [shard_path]
...
#             'giggle', 'index',
#             '-i', f"{shard_path}/*",
#             '-o', os.path.join(args.output, f'{args.prefix}{i}', args.index),
#         ]
