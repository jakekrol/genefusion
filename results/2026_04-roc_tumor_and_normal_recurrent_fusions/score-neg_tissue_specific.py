...
# example:
# fusion: erg-tmprss2
# recurrent tissue: prostate
# tissue scoring: prostate
...
parser.add_argument("--outfile", default="score-neg_tissue_specific.tsv")
parser.add_argument("--logfile", default="score-neg_tissue_specific.log")
...
# get a set of all pcawg tissues
all_pcawg_tissues = df_evidence.columns.tolist()[2:]
# remove 1000g from pcawg tissue set
...
for i, row in df_fusions.iterrows():
	...
	### get evidence as positive
	for w_normal in w_normals:
		tissues = row["tissue_w_data"]
		# in total there will be 2 + 2*len(off_tissue) colmap pairs for each fusion
		# the first 2 colmap pairs will be for scoring the fusion as positive (in-tissue) with and without 1000g evidence
		# the rest of the colmap pairs will be for scoring the fusion as negative (off-tissue) with and without 1000g evidence for each off-tissue
		colmaps = []
		### get evidence as positive (in-tissue)
		...
		### get evidence as negative (off-tissue)
		for t in off_tissues:
			...
			_, normal_colmap_i_neg_w_1000g = colmap_subset(tumor_colmap, normal_colmap, [t], include_1000g=True)
			colmaps.append((tumor_colmap_i_neg, normal_colmap_i_neg_w_1000g, f"score_as_negative-tissue_{t}-w_1000g-wnormal_{w_normal}"))
		# score fusion with various evidence type subsets
